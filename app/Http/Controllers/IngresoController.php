...
class IngresoController extends Controller
{
  public function index(Request $request){
    ...
    if ($buscar==''){
      ...
      'ingresos.num_comprobante','ingresos.fecha_hora','ingresos.impuesto','ingresos.total',
      'ingresos.estado','personas.nombre','users.usuario')
      ->where('ingresos.'.$criterio, 'ILIKE', '%'. $buscar . '%')->orderBy('ingresos.id', 'desc')->paginate(3);
    }
  ...
  public function store(Request $request){
    ...
    try{
      ...
      DB::commit();
      $now = date('Y-m-d');
      $numVentas = DB::table('ventas')->whereDate('created_at', $now)->count();
      $numIngresos = DB::table('ingresos')->whereDate('created_at', $now)->count();
      ...
      foreach ($allUsers as $notificar) {
        User::findOrFail($notificar->id)->notify(new NotifyAdmin($arr_data));
      }
    } catch (Exception $e){
      DB::rollBack();
    }
